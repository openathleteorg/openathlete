...
/**
 * Adapter component for duration field that manages local state for hours, minutes, and seconds
 */
function DurationFieldAdapter({
  field,
  error,
  onChange,
  showSeconds,
  ...
  const [hoursInput, setHoursInput] = useState<string>(
    field.value ? Math.floor((field.value as number) / 3600).toString() : '',
  );
  const [minutesInput, setMinutesInput] = useState<string>(
    field.value
      ? Math.floor(((field.value as number) % 3600) / 60).toString()
      : '',
  );
  const [secondsInput, setSecondsInput] = useState<string>(
    showSeconds && field.value
      ? Math.floor((field.value as number) % 60).toString()
      : '',
  );
  ...
  useEffect(() => {
    if (field.value === undefined || field.value === null) {
      ...
      setHoursInput(Math.floor((field.value as number) / 3600).toString());
      setMinutesInput(
        Math.floor(((field.value as number) % 3600) / 60).toString(),
      );
      if (showSeconds) {
        setSecondsInput(Math.floor((field.value as number) % 60).toString());
      }
    }
